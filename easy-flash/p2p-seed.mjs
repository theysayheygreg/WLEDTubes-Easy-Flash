...
export function createP2PSeedRuntime({delay=ms=>new Promise(resolve=>setTimeout(resolve,ms)),bootSettleMs=5000,timeoutMs=5000}={}) {
  ...
  async function command(port,{onStatus=()=>{}}={}) {
    onStatus("Opening the connected controller serial session…");await port.open({baudRate:115200});let reader;
    try {
    ...
    } finally {try{await reader?.cancel();}catch{}try{reader?.releaseLock();}catch{}try{await port.close();}catch{}}
  }
  async function seed(candidate,options={}) {
    ...
    try{return await command(binding.port,options);}finally{binding=null;}
  }
  async function seedConnected({serial,onStatus=()=>{}}={}) {
    if(!serial?.requestPort)throw Error("USB serial is unavailable in this app");
    const port=await serial.requestPort();
    const result=await command(port,{onStatus});
    if(!result.acknowledged)throw Error("The connected firmware did not accept a Dig2Go propagation turn");
    return result;
  }
  return {bind,seed,seedConnected};
}
